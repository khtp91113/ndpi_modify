...
static time_t rest_inter = 0;
static u_int8_t rest_flag = 0;
...
static void parseOptions(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "df:g:i:hp:l:s:tv:V:n:j:rp:w:qR:")) != EOF) {
    switch (opt) {
    case 'd':
	...
	case 'R':
	  rest_flag = 1;
	  rest_inter = atoi(optarg);
	  printf("send json every %d sec\n", (int)rest_inter);
	  break;
...
static void printResults(u_int64_t tot_usec) {
  ...
  if(json_flag != 0) {
	...
	json_object_put(jObj_main); // free json objects
    fclose(json_fp);
#endif
  }
}
...
void test_lib() {
  ...
  /* test reset */
	...
	puts("-------- reset flow tree ----------");
	ndpi_flowtree_reset(ndpi_thread_info[0].workflow);
	puts("------------------------------");
  printResults(tot_usec);
