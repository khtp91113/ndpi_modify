...
void ndpi_flowtree_reset(struct ndpi_workflow* workflow) {
	int i;

	memset(&workflow->stats, 0, sizeof(struct ndpi_stats));
	for(i=0; i<workflow->prefs.num_roots; i++)
		ndpi_tdestroy(workflow->ndpi_flows_root[i], ndpi_flow_info_freer);
	puts("finish reset");
	free(workflow->ndpi_flows_root);
  ...
  *proto = iph->protocol;
  ...
  if(ret == NULL) { // not exist
    ...
    if(workflow->stats.ndpi_flow_count == workflow->prefs.max_ndpi_flows) {
      ...
      struct ndpi_flow_info *newflow = (struct ndpi_flow_info*)malloc(sizeof(struct ndpi_flow_info)); // alloc memory to newflow
	...
	return(NULL);
	  ...
	  /* custom fields */
      strncpy(newflow->mac_addr.src_mac, ndpi_mac.src_mac, 19); // add mac address
	  strncpy(newflow->mac_addr.dst_mac, ndpi_mac.dst_mac, 19);
	...
	inet_ntop(AF_INET6, &iph6->ip6_dst, newflow->upper_name, sizeof(newflow->upper_name));
	  ...
	  /*
	   * prepare ndpi_flow_struct in ndpi_flow_info
	   * */
	...
	memset(newflow->ndpi_flow, 0, SIZEOF_FLOW_STRUCT); // alloc ndpi_flow_struct => alloc detected_protocol_stack // clear bitmask!!!!
	...
	memset(newflow->dst_id, 0, SIZEOF_ID_STRUCT);
      ...
      *src = newflow->src_id, *dst = newflow->dst_id; // newflow (ndpi_flow_info.src_id == ndpi_flow_struct.src)
...
static struct ndpi_flow_info *get_ndpi_flow_info6(struct ndpi_workflow * workflow,
  ...
  return(get_ndpi_flow_info(workflow, 6, vlan_id, &iph, iph6, ip_offset,
			    ...
			    src, dst, proto, payload, payload_len, src_to_dst_direction, ndpi_mac));
}
...
static unsigned int packet_processing(struct ndpi_workflow * workflow,
					  ...
					  struct macAddr ndpi_mac) {
  struct ndpi_id_struct *src, *dst; // get instance in function get_ndpi_flow_info()
  struct ndpi_flow_info *flow; // get instance in function get_ndpi_flow_info()
  ...
  if(iph)
    flow = get_ndpi_flow_info(workflow, 4, vlan_id, iph, NULL, // insert flow node and return address
			      ip_offset, ipsize,
			      ntohs(iph->tot_len) - (iph->ihl * 4),
			      &tcph, &udph, &sport, &dport,
			      &src, &dst, &proto,
			      &payload, &payload_len, &src_to_dst_direction, ndpi_mac);
  else
  ...
  /* accumulate packet and size info to stats */
  if(flow != NULL) {
    ...
    return(0); // failed to alloc memory
  }

  /* Protocol already detected */
  if(flow->detection_completed) return(0); // if the flow isn't newflow, the protocol should be detected before.
  ...
  // print http url info
	/*if(ndpi_flow->packet.http_url_name.ptr!=NULL)*/
		/*printf("URL %s\n", ndpi_flow->packet.http_url_name.ptr);*/

  if((flow->detected_protocol.protocol != NDPI_PROTOCOL_UNKNOWN) // decide the flow protocol type.
     || ((proto == IPPROTO_UDP) && (flow->packets > 8)) // packets are many enough, but ndpi can't decide protocol type.
     || ((proto == IPPROTO_TCP) && (flow->packets > 10))) {
    /* New protocol detected or give up */
    flow->detection_completed = 1; // stop detection // giveup OR done
    ...
    if(flow->detected_protocol.protocol == NDPI_PROTOCOL_UNKNOWN) {
		/* giveup check */
      flow->detected_protocol = ndpi_detection_giveup(workflow->ndpi_struct, flow->ndpi_flow); // proto.txt: ip based protocol
	...
	/* not detect forever, free flow's determinant data */
    ndpi_free_flow_info_half(flow);
  }
...
void ndpi_workflow_process_packet (struct ndpi_workflow * workflow,
 ...
 datalink_check:
  ...
  case DLT_EN10MB :
	...
	snprintf(ndpi_mac.dst_mac, sizeof(ndpi_mac.dst_mac), "%02x:%02x:%02x:%02x:%02x:%02x",
			 (ethernet->h_dest)[0], (ethernet->h_dest)[1], (ethernet->h_dest)[2], (ethernet->h_dest)[3], (ethernet->h_dest)[4], (ethernet->h_dest)[5]);
	snprintf(ndpi_mac.src_mac, sizeof(ndpi_mac.src_mac), "%02x:%02x:%02x:%02x:%02x:%02x",
 ...
 iph_check:
  ...
  packet_processing(workflow, time, vlan_id, iph, iph6,
		    ip_offset, header->len - ip_offset, header->len, ndpi_mac);
